# Corine Land Cover CLC

- Source: European Union's Copernicus Land Monitoring Service information (2020)
- Download links:
  https://land.copernicus.eu/en/products/corine-land-cover/clc-2000
  https://land.copernicus.eu/en/products/corine-land-cover/clc-2006
  https://land.copernicus.eu/en/products/corine-land-cover/clc-2012
  https://land.copernicus.eu/en/products/corine-land-cover/clc2018
   
- Expected files to leave in the folder: 
  U2006_CLC2000_V2020_20u1.tif 
  U2012_CLC2006_V2020_20u1.tif 
  U2018_CLC2012_V2020_20u1.tif 
  U2018_CLC2018_V2020_20u1.tif

Download the data from the source above and place the files directly in this
folder (`Databases/CLC_all_years/`) so the scripts can find them at the paths they expect.
